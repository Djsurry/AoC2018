	...
	def __init__(self, _id, data):
		...
		for i in self.records:
			if "falls asleep" in i:
				fell_asleep = i[1:17].split(":")[1]
			else:
				woke_up = i[1:17].split(":")[1]
			
				for i in range(int(fell_asleep), int(woke_up)):
					self.sleeping_minutes.append(i)
		
		if len(self.sleeping_minutes) == 53:
			print(self.records)
			print(self.id)
...
# "id": '', "data": []
guards = []
for i in sorted(inp.split('\n')):
	if "Guard" in i:
		if lastGuard:
			guards.append(lastGuard)
		s = i.index("#")
		_id = ''
		ind = 0
		while True:
			...
			if i[s + ind] != ' ':
				...
				break
		lastGuard = {"id": _id, "data": []}
	else:
		lastGuard["data"].append(i)
guards = [Guard(n["id"], n["data"]) for n in guards]

biggest = None
for guard in guards:
	if not biggest or len(biggest.sleeping_minutes) < len(guard.sleeping_minutes):
		biggest = guard
seen = []
for i in biggest.sleeping_minutes:
	if i in seen:
		dup = i
		break
	else:
		seen.append(i)
print(biggest.sleeping_minutes)
...
totals = []
minutes = []
for record in sorted(inp.split('\n')):
	if "begins shift" in record:
		...
		while True:
			...
			if record[i] != ' ':
				...
				break
		if lastGuard:
			totals.append({"asleep": minutes, "id": lastGuard})
		minutes = []
		lastGuard = _id
	else:
		if "wakes" in record:
			index = sorted(inp.split('\n')).index(record)
			saved = []
			offset = 0

			while True:
				if "sleep" in sorted(inp.split('\n'))[index-offset]:
					break
				else:
					
					minutes.append(sorted(inp.split('\n'))[index-offset][1:17].split(":")[1])
					offset += 1
			

biggest = {"asleep": []}
for i in totals:
	if len(i["asleep"]) > len(biggest["asleep"]):
		biggest = i


seen = []
for i in biggest["asleep"]:
	if i in seen:
		dup = i
		break
	seen.append(i)

print(int(dup)*int(biggest["id"]))
